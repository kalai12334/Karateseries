Feature: GET API headers feature
  ...
  Scenario: pass the user request with headers_Part2
    * def request_h = {Content-Type: 'text/xml;charset=ISO-8859-1', Connection: 'Keep-Alive', User-Agent: 'Mozilla/4.0(compatible;IE;GACv7\. 1\. 5192\. 22378)'}
    Given headers request_h
    When url baseUrl+'/public/v1/users'
    And path '30'
    When method GET
    Then status 200
